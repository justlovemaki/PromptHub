...
export const prompt = pgTable("prompt", {
	...
	approvalStatus: approvalStatusEnum("approval_status").default("PENDING"), // 审核状态：PENDING-待审核, APPROVED-已通过, REJECTED-已拒绝
...
export const promptFavorite = pgTable("prompt_favorite", {
	id: text("id").primaryKey().notNull(),
	userId: text("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
	promptId: text("prompt_id").notNull().references(() => prompt.id, { onDelete: "cascade" }),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
